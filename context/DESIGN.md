...
- **Hero Section (Immersive Background Video):** A clean, highly performant layout featuring a powerful static narrative on the left and a continuous, looping video (`logo-animation.mp4`) serving as an immersive full-bleed background. The video autoplays seamlessly on the right side and blends into the white background on the left using soft gradient edge masks (fade-to-white horizontally and vertically). This eliminates restrictive frames or shadow boxes, creating an advanced, premium, and professional aesthetic without complex scroll-jacking (GSAP), ensuring instantaneous load times and zero lag across all devices.
